...
import br.edu.utfpr.trabalhofinal.utils.verificaCorPorTipoValor
    ...
    LazyColumn(modifier = modifier) {
        items(contas) { conta ->
            ListItem(
                ...
                headlineContent = {
                    Text(text = conta.descricao, fontWeight = FontWeight.SemiBold)
                },
                supportingContent = {
                    Row(
                        ...
                        Text(
                            text = (if (conta.tipo == TipoContaEnum.DESPESA) "-" else "") + conta.valor.formatar(),
...
private fun BottomBar(
    ...
    Column(
        ...
        Totalizador(
            ...
            textColor = contas.calcularSaldo().verificaCorPorTipoValor()
        )
        Totalizador(
            modifier = Modifier.padding(bottom = 20.dp),
            titulo = stringResource(R.string.previsao),
            valor = contas.calcularProjecao(),
            textColor = contas.calcularProjecao().verificaCorPorTipoValor()
        )
    }
}
